...
function computePath(cx: number, cy: number, t: number, s: AudioSettings, rms: number): string {
...
export default function CircleVisualizer({ settings, isPlaying, getSmoothedRms }: Props) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      pathRef.current?.setAttribute('d', computePath(150, 150, t, s, rms));
      ...
      const glowPx = (3 + rms * 14).toFixed(1);
